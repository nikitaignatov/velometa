...
void main_screen(void)
{
  ...
  /*Tile2: a button*/
  lv_obj_t *scan_tile = lv_tileview_add_tile(tv, 1, 0, LV_DIR_LEFT | LV_DIR_RIGHT);
  lv_obj_t *map_tile = lv_tileview_add_tile(tv, 2, 0, LV_DIR_LEFT);
  ...
  // LV_IMG_DECLARE(p20478);

  img = lv_img_create(map_tile);
  lv_img_set_antialias(img, false);
  // lv_img_cache_invalidate_src(NULL);
  // lv_img_set_src(img, &p20478);
  lv_img_set_src(img, "S:/p_256_20478_4bit.bin");
